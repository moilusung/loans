...
<html lang="en" dir="ltr">
  ...
  <body>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          ...
          <li class="nav-item active">
            <a class="nav-link text-dark" href="nfiswriteoff.php"><b>Write Off <span class="sr-only">(current)</span></b></a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="nfislitigation.php">Items In Litigation</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="nfisForeclosed.php">Foreclosed</a>
          </li>
        </ul>
      </div>
    </nav>
   ...
   <form>
         ...
         <tr style="line-height: 8px" class=" text-dark text-center">
           ...
           <td class="table-success text-dark" width="7%"  class=""><font size="1px">WrittenoffDate</font></td>
       ...
       <tbody>
         <?php
             ...
             $sql="SELECT nfis.ReportingBranch,
                      		...
                      		nfis.OSBalance,
                      		FORMAT(nfis.WrittenoffDate,'MM-dd-yyyy') WrittenoffDate,
                      	...
                      	FROM nfiswrittenoffindiv nfis
                      	INNER JOIN Cyber1.dbo.loanLedger lle ON lle.AccountNumber = nfis.LoanReferenceNo
                      	INNER JOIN Cyber1.dbo.loanStatus lst ON lst.StatusID = lle.LoanStatusID
                      	INNER JOIN
                      	(
                      	...
                      	FROM Cyber1.dbo.loanTransaction ltr
                      ...
                      UNION
                      	SELECT nfis.ReportingBranch,
                      		...
                      		FORMAT(ltr.TransactionDateTime,'MM-dd-yyyy') TransactionDateTime
                      	FROM nfiswrittenoffindiv nfis
                      	INNER JOIN Cyber2.dbo.loanLedger lle ON lle.AccountNumber = nfis.LoanReferenceNo
                      	INNER JOIN Cyber2.dbo.loanStatus lst ON lst.StatusID = lle.LoanStatusID
                      	INNER JOIN
                      	(
                      	SELECT ltr.TransactionDateTime,
                      		ltr.AccountNumber,
                      		ltr.TransactionNumber,
                      		ROW_NUMBER() OVER (PARTITION BY ltr.AccountNumber ORDER BY ltr.TransactionNumber DESC ) AS RowNum
                      	FROM Cyber2.dbo.loanTransaction ltr
                      	WHERE DATEDIFF(day,ltr.TransactionDateTime,'2021-08-24') >= 1
                      	AND ltr.TransactionType <> 'Transfer'
                      	) ltr ON ltr.AccountNumber = nfis.LoanReferenceNo AND ltr.RowNum = 1
                      	WHERE lle.PrincipalBalance = 0
                      		OR lle.LoanStatusID = 1
                      UNION
                      	SELECT nfis.ReportingBranch,
                      		...
                      		FORMAT(ltr.TransactionDateTime,'MM-dd-yyyy') TransactionDateTime
                      	FROM nfiswrittenoffindiv nfis
                      	INNER JOIN Cyber3.dbo.loanLedger lle ON lle.AccountNumber = nfis.LoanReferenceNo
                      	INNER JOIN Cyber3.dbo.loanStatus lst ON lst.StatusID = lle.LoanStatusID
                      	INNER JOIN
                      	(
                      	SELECT ltr.TransactionDateTime,
                      		ltr.AccountNumber,
                      		ltr.TransactionNumber,
                      		ROW_NUMBER() OVER (PARTITION BY ltr.AccountNumber ORDER BY ltr.TransactionNumber DESC ) AS RowNum
                      	FROM Cyber3.dbo.loanTransaction ltr
                      	WHERE DATEDIFF(day,ltr.TransactionDateTime,'2021-08-24') >= 1
                      	AND ltr.TransactionType <> 'Transfer'
                      	) ltr ON ltr.AccountNumber = nfis.LoanReferenceNo AND ltr.RowNum = 1
                      	WHERE lle.PrincipalBalance = 0
                      ...
                      UNION
                      	SELECT nfis.ReportingBranch,
                      		...
                      		FORMAT(ltr.TransactionDateTime,'MM-dd-yyyy') TransactionDateTime
                      	FROM nfiswrittenoffindiv nfis
                      	INNER JOIN Cyber4.dbo.loanLedger lle ON lle.AccountNumber = nfis.LoanReferenceNo
                      	INNER JOIN Cyber4.dbo.loanStatus lst ON lst.StatusID = lle.LoanStatusID
                      	INNER JOIN
                      	(
                      	SELECT ltr.TransactionDateTime,
                      		ltr.AccountNumber,
                      		ltr.TransactionNumber,
                      		ROW_NUMBER() OVER (PARTITION BY ltr.AccountNumber ORDER BY ltr.TransactionNumber DESC ) AS RowNum
                      	FROM Cyber4.dbo.loanTransaction ltr
                      	WHERE DATEDIFF(day,ltr.TransactionDateTime,'2021-08-24') >= 1
                      	AND ltr.TransactionType <> 'Transfer'
                      	) ltr ON ltr.AccountNumber = nfis.LoanReferenceNo AND ltr.RowNum = 1
                      	WHERE lle.PrincipalBalance = 0
           ...
           if (($stmt = sqlsrv_query($conn, $sql,$param)) === false) {
               ...
               echo '<td><font size="1px">'. number_format($row['PastDueAmount'],2) .'</font></td>';
               echo '<td><font size="1px">'. $row['PastDueDate'] .'</font></td>';
  ...
  $(document).ready(function(){
      $("select").on("change", function(){
          ...
          $.post("nfiswriteoffupdate.php", {account: LoanReferenceNo, status: statusID}, function(response){
              alert(response);
          })
      });
  });
